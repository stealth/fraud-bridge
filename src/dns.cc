...
int DNS::query(const string &host, string &result, uint16_t qtype)
{
	...
	qtype = htons(qtype);
	memcpy(buf + idx, &qtype, sizeof(uint16_t));
	idx += sizeof(uint16_t);

	uint16_t tmp = htons(1);	// INET
	memcpy(buf + idx, &tmp, sizeof(uint16_t));
	idx += sizeof(uint16_t);
...
int DNS::add_ns(const string &host, const string &port = "53")
{
	...
	hints.ai_family = d_family;
	hints.ai_socktype = SOCK_DGRAM;

 	int e;
	if ((e = getaddrinfo(host.c_str(), port.c_str(), &hints, &ai)) < 0) {
		d_err = "DNS::add_ns:getaddrinfo:";
		d_err += gai_strerror(e);
		return -1;
	}

	d_ns_map[ai] = ai->ai_addrlen;
	return 0;
}
...
int DNS::parse_query(const string &msg, string &packet, const sockaddr *from)
{
	...
	const dnshdr *hdr = reinterpret_cast<const dnshdr *>(msg.c_str());
	...
	if ((nl = qname2host(string(qname, msg.size() - sizeof(dnshdr)), fqdn, d_enc_domain)) < 0)
		return -1;

	if (sizeof(dnshdr) + nl + 4 > msg.size() || fqdn.find(d_domain) == string::npos)
		return build_error("parse_query: Invalid query (1)");
	string::size_type domain_start = 0;
	if ((domain_start = fqdn.find("." + d_domain)) == string::npos)
...
int DNS::txt_response(const string &msg, string &result, sockaddr *to)
{
	...
	memcpy(buf + idx, Q.Q_string.c_str(), Q.Q_string.size());
	idx += Q.Q_string.size();
	...
	memcpy(buf + idx, &compressed_lbl, sizeof(compressed_lbl));
	idx += sizeof(compressed_lbl);
	memcpy(buf + idx, &type, sizeof(type));
	idx += sizeof(type);
	memcpy(buf + idx, &cl, sizeof(cl));
	idx += sizeof(cl);
	memcpy(buf + idx, &ttl, sizeof(ttl));
	idx += sizeof(ttl);
	memcpy(buf + idx, &rdlen, sizeof(rdlen));
	rdlen_ptr = reinterpret_cast<uint16_t *>(buf + idx);
	idx += sizeof(rdlen);
	...
	rdlen = htons(rdlen);
	memcpy(rdlen_ptr, &rdlen, sizeof(rdlen));
...
int DNS::parse_txt_response(const string &msg, string &packet)
{
	...
	for (uint16_t i = 0; i < ntohs(hdr->a_count); ++i) {
		...
		type_ptr = reinterpret_cast<const uint16_t *>(idx);
		rdlen_ptr = reinterpret_cast<const uint16_t *>(type_ptr + 4);	// 4 16bit words

		if (ntohs_ua(type_ptr) != dns_type::TXT)
			return build_error("parse_txt_response: invalid packet (5)");
		if (ntohs_ua(rdlen_ptr) == 0)
			return build_error("parse_txt_response: invalid packet (6)");
		txt_rr = reinterpret_cast<const char *>(rdlen_ptr + 1);

		if (txt_rr + ntohs_ua(rdlen_ptr) > msg.c_str() + msg.size())
		...
		for (uint16_t j = 0; j < ntohs_ua(rdlen_ptr);) {
			if ((olen = txt_octets[0]) == 0)
				break;
			if (txt_octets + 1 + olen > txt_rr + ntohs_ua(rdlen_ptr))
				return build_error("parse_txt_response: invalid packet (8)");;
			s += string(txt_octets + 1, olen);
			txt_octets += olen + 1;
			j += olen + 1;
		}
		idx += ntohs_ua(rdlen_ptr);
	}
...
{
	if (d_sock < 0) {
		if ((d_sock = socket(d_family, SOCK_DGRAM, 0)) < 0)
			return build_error("send::socket");
	}

	for (auto i = d_ns_map.begin(); i != d_ns_map.end(); ++i)
		sendto(d_sock, msg.c_str(), msg.length(), 0, i->first->ai_addr, i->second);
	return 0;
}
...
{
	while (d_last_Qs.size() > 3)
		d_last_Qs.pop_front();

	while (d_last_Qs.size() > 0 && i--)
		d_last_Qs.pop_front();
}
